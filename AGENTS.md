...
## オーケストレーション・エージェントの役割 (Agent Roles)

添付図に示されるオーケストレーション・アーキテクチャでは、以下の役割を持つエージェントが連携します。

- **Conductor (指揮者)**: 全行程の進行管理。ユーザーとの対話、各フェーズの実行指示、最終報告を担当します。
- **Architect (設計者)**: システム設計。ユーザーの要件を元に実装計画 (`implementation_plan.md`) を作成します。
- **Developer (開発者)**: ロジックやバックエンドの実装。
- **Creator (制作者)**: UI/UX デザイン、フロントエンドの制作、アセット作成。
- **Reviewer (レビュー担当)**: コード品質の確認。複数のエージェントが並行してフィードバックを提供します。
- **Tester (テスト担当)**: 動的検証、テストコードの実行、機能の動作確認。

## 新しいエージェント（スキル）を追加する手順

1. `skills/` 以下に新しいスキルディレクトリを作成し、`SKILL.md` と必要な設定ファイルを追加する。
2. `src/domain/models/` と `src/usecases/` の該当箇所にビジネスロジック（インターフェース）を追加する。
3. `src/cli/cli-config.yaml` または `.kanon/config.json` に skill→CLI マッピングを追加し、`mergeJsonConfig()` での互換性を確認する。
4. ユニットテスト（`tests/skills/<your-skill>.test.ts`）を追加して、ワークフローに割り込ませるモックを用意する。

## シークレットと認証情報の取り扱い

- 絶対に API キー / トークンをリポジトリにコミットしないでください。
- LLM CLI の認証情報はシステムの資格情報ストア、またはCI のシークレット管理機能（GitHub Actions Secrets など）を利用して注入してください。
- 開発時は `.env` を使う場合、`.gitignore` に追加し、サンプルは `.env.example` に置いてください。

## セキュリティ注意点

- 設定ファイルは外部から提供されるとコマンド実行に影響します。`.kanon/config.json` の内容は必ず信頼できるソースからのみ適用してください。
- `cli-resolver` のように外部コマンドを起動する箇所は将来的にホワイトリスト検証を追加してください。
