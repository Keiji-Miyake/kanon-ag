...
license: MIT License
metadata:
  author: Kanon Project
  version: "1.0"
---

# Project Scaffold Skill

このスキルは、AI駆動開発に対応した新規プロジェクト環境を構築するための手順と知識を提供します。
...
このプロジェクトでは「ドキュメント駆動開発」と自律型エージェントワークフローを採用しています。
...
### 4.- **プロジェクト構造の初期化**: 新しい機能開発やプロジェクト開始時に、標準的なディレクトリ構造と空のドキュメントセットを生成する

- **一貫性の維持**: プロジェクトごとのルールや慣習に従った命名と配置を行う。

## ワークフロー

### 1. 構造の決定

- プロジェクトの標準ルール（`AGENTS.md` や既存のディレクトリ構成）を確認し、作成すべきディレクトリ構造を決定する。
- ユーザーからの指定があればそれに従う。

### 2. ディレクトリとファイルの生成

- 必要なディレクトリを作成（例: 機能別ドキュメントフォルダ）。
- 必須ドキュメント（要件定義書、設計書、コンテキストログなど）の空ファイルまたはテンプレートを作成。

### 3. 初期化

- 作成したファイルのヘッダーにタイトルや目的を記述し、スムーズに作業を開始できる状態にする。
