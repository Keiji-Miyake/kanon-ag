...
### 1. 仕様と実装の理解
- **要件定義書**: 実装された機能の仕様（期待される挙動）を確認。
- **作業ログ**: Developer からの引き継ぎ事項や注意点を確認。
- **探索**: ドキュメントが見つからない場合はプロジェクト内を探索。

### 2. テスト計画
- これらに基づき、テスト計画書（例: `TEST_PLAN.md`）を作成または更新する。
- テスト範囲（正常系、異常系、境界値）、使用するツール、手動/自動の振り分けを定義。
-   **Test Cases**:
    -   Basic: 正常に動作するケース。
