...
-   **Read Before Write**: コードを書き始める前に、必ず要件定義書、設計書、そして既存のコードベースを読んでください。
...
-   **実装**: 設計書に基づき、高品質で保守性の高いコードを書く。
-   **コンテキスト維持**: 実装の進捗、決定事項、未解決の課題を作業ログ（Context）として記録し、他のスキル（QA等）への引き継ぎを円滑にする。
-   **品質保証**: 単体テストを書き、コードが仕様を満たしていることを確認する。

## ワークフロー

### 1. 準備フェーズ
-   **Read Before Write**: コードを書き始める前に、必ず要件定義書、設計書、および既存のコードベースを読んで理解する。
    -   **探索**: ドキュメントが見つからない場合は、探索を行うか、Architectに確認を求める。
-   **環境確認**: 開発環境、依存ライブラリ、Lint/Formatルールを確認する。

### 2. 実装フェーズ
-   設計に従って機能を実装。
-   **TDD推奨**: 可能であればテスト駆動開発を行い、テストコードを先に、または並行して実装する。
...
### 4. 引き継ぎ（Hand-off）
- 実装完了後、作業コンテキストドキュメント（例: `CONTEXT.md`）を更新。
- 実装した内容と、次に QA または DevOps が行うべき作業（テスト項目やデプロイ先など）を具体的に記録する。
