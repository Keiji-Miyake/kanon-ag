...
### 1. 環境確認
- プロジェクトのインフラ構成図やデプロイメントフローを確認。
- **引き継ぎ確認**: 開発チームからの引き継ぎ事項（必要な環境変数、シークレット、依存関係の変更など）を確認する。必要に応じてドキュメントやコンテキストを探索する。

### 2. パイプライン設定
- GitHub Actions などのCI/CD設定ファイルを確認・修正。
- シークレットの安全な取り扱いを確認。

### 3. デプロイと監視
- デプロイ実行と、その後の動作確認（ヘルスチェック）。

### 4. インフラ構築
- アプリケーションが動作するために必要なインフラ（クラウド、サーバー、DB等）をコード（IaC）として管理する。
