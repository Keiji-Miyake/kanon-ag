 ...
 */
export class AiJudgeClient implements IAiJudgeClient {
    /**
     * @param runner A function that takes a prompt and returns the agent's output string.
     */
    constructor(
        private runner: (prompt: string) => Promise<string>
    ) {}
     ...
     */
    public async evaluate(context: string): Promise<EvaluationResult> {
        const systemPrompt = `
...
`;
    ...
    private parseResult(output: string): EvaluationResult {
    ...
    private mapToEvaluationResult(data: any): EvaluationResult {
        return {
            status: (['CONTINUE', 'ABORT', 'ESCALATE'].includes(data.status) ? data.status : 'CONTINUE') as any,
