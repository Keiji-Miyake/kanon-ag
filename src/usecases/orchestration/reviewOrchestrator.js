import { MergeGateway } from '../../domain/services/mergeGateway.js';
import { FeedbackInjector } from '../prompt/feedbackInjector.js';
export class ReviewOrchestrator {
    parallelReviewers;
    agentRunner;
    onStatusUpdate;
    constructor(_sessionId, parallelReviewers, agentRunner, onStatusUpdate) {
        this.parallelReviewers = parallelReviewers;
        this.agentRunner = agentRunner;
        this.onStatusUpdate = onStatusUpdate;
    }
    emitStatus(message, metadata = {}) {
     ...
     */
    async startAsyncReviews(worktreePath, currentInstruction) {
        const reviewPromises = this.parallelReviewers.map(agentId => this.agentRunner.runReview(agentId, worktreePath, currentInstruction));
     ...
     */
    evaluateFeedbacks(feedbacks, condition) {
     ...
     */
    async runCorrectionLoop(developerId, worktreePath, initialInstruction, condition, maxRetries = 3, gatekeeperFn, interventionProvider) {
        let currentInstruction = initialInstruction;
        let attempt = 0;
        const injector = new FeedbackInjector();
        while (attempt < maxRetries) {
            // 0. ユーザーからの介入（口出し・割り込み）をチェック
            if (interventionProvider) {
                const userIssues = await interventionProvider();
                if (userIssues && userIssues.length > 0) {
                    this.emitStatus(`User intervention detected! Injecting user feedback...`, { attempt: attempt + 1, userIssues });
                    currentInstruction = injector.injectIssues(currentInstruction, userIssues);
                    // 介入があった場合は、即座に次の（修正された）実装ステップへ進む
                }
            }
