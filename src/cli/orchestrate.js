...
// 新アーキテクチャインポート群
import { OrchestrationService } from '../usecases/orchestration/orchestrationService.js';
// Global logger for real-time streaming to UI
export class Logger {
    static clients = new Set();
    static wsClient = null;
    static wss = null;
    static isUiServer = false;
    static logBuffer = [];
    static interventionBuffer = [];
    ...
    static setUiServerMode(isServer) {
        this.isUiServer = isServer;
    }
    static setCurrentSessionId(id) {
        this.currentSessionId = id;
    }
    static async pollIntervention() {
        if (this.interventionBuffer.length === 0)
            return null;
    ...
    static handleIncomingMessage(data, sender) {
        try {
    ...
    static log(message, agent = 'kanon', metadata = {}) {
...
async function runResumeScore(args) {
    Logger.log('Checking for interrupted score session...', 'Conductor');
    const session = readSession();
    if (!session || session.command !== 'score') {
        Logger.log('⚠️ 再開可能な Score セッションが見つかりません。', 'system');
        return;
    }
    if (session.status === 'completed') {
        Logger.log('✅ 前回の Score セッションは正常に完了しています。', 'system');
        return;
    }
    await runScore(args, session);
}
async function main() {
...
Commands:
  ...
  resume    Resume an interrupted session (plan/execute/run)
  score     Execute Score-based autonomous loop (use --resume to resume)
...
async function runHistory(_args) {
...
async function runExecute(args) {
    ...
    const service = new OrchestrationService((msg, agent, meta) => Logger.log(msg, agent, meta));
    await service.runExecute(taskName, taskForResume, sessionId);
}
async function runResume(_args) {
    ...
    switch (command) {
        ...
        case 'run': {
            ...
            if (phase === 'run-plan' || phase === 'plan') {
                ...
                try {
                    updateSession('running', 'run-plan', undefined, 'run', targetTask);
                }
                catch (_) { /* ignore */ }
                await runPlan(resumeArgs);
                try {
                    updateSession('running', 'run-execute', undefined, 'run', targetTask);
                }
                catch (_) { /* ignore */ }
                await runExecute(resumeArgs);
...
// ────────────────────────────────────────────────────────────
function ensureSession(command, targetTask, phase) {
 ...
 * Score に基づいた自律実行。
 */
async function runScore(args, resumeSession) {
    const service = new OrchestrationService((msg, agent, meta) => Logger.log(msg, agent, meta));
    await service.runScore(args, resumeSession);
}
// Executed directly
main();
