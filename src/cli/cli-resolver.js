    ...
    const defaultConfig = {
        ...
        agents: {
            ...
            developer: {
                ...
                model_backups: ['github-copilot/claude-sonnet-4.6', 'github-copilot/gpt-5.4'],
                cli_backup: 'copilot',
                cli_backup_model: 'github-copilot/gpt-5-mini'
            },
            creator: {
                command: 'copilot',
                model: 'gpt-5-mini',
                model_backups: ['gpt-4.1'],
                cli_backup: 'gemini',
                cli_backup_model: 'gemini-2.5-flash-lite'
            },
            tester: {
                command: 'copilot',
                model: 'gpt-5-mini',
 ...
 */
export function mergeJsonConfig(base, json) {
    const merged = {
        ...base,
    ...
    // 古い形式の agentModelMapping を skill_cli_mapping にマージ
    if (json.agentModelMapping && typeof json.agentModelMapping === 'object') {
        const mapping = json.agentModelMapping;
        for (const [skill, cli] of Object.entries(mapping)) {
            merged.skill_cli_mapping[skill] = cli;
        }
    }
    // agents セクションのマージ
    if (json.agents && typeof json.agents === 'object') {
        for (const [role, agentCfg] of Object.entries(json.agents)) {
            if (!agentCfg || typeof agentCfg !== 'object')
                continue;
            const cfg = agentCfg;
            const current = merged.agents[role] || { cli: 'gemini', model_primary: '' };
            const defaultConfig = getDefaultConfig().agents[role] || { cli: 'gemini', model_primary: '' };
            const cli = (typeof cfg.command === 'string' ? cfg.command : (typeof cfg.cli === 'string' ? cfg.cli : current.cli));
            const model = typeof cfg.model === 'string' ? cfg.model : (typeof cfg.model_primary === 'string' ? cfg.model_primary : current.model_primary);
            merged.agents[role] = {
                cli,
                model_primary: model,
                model_backup: typeof cfg.model_backup === 'string' ? cfg.model_backup : (current.model_backup || defaultConfig.model_backup),
                model_backups: Array.isArray(cfg.model_backups) ? cfg.model_backups : (current.model_backups || defaultConfig.model_backups),
                cli_backup: typeof cfg.cli_backup === 'string' ? cfg.cli_backup : (current.cli_backup || defaultConfig.cli_backup),
                cli_backup_model: typeof cfg.cli_backup_model === 'string' ? cfg.cli_backup_model : (current.cli_backup_model || defaultConfig.cli_backup_model),
            };
            // 後方互換性のためマッピングも更新
            merged.skill_cli_mapping[role] = cli;
            if (!merged.agentModels)
                merged.agentModels = {};
            merged.agentModels[role] = model;
        }
    }
    return merged;
}
...
function parseYaml(content) {
    ...
    for (const rawLine of lines) {
        ...
        if (indent === 0 && trimmed.includes(':')) {
            ...
            if (key === 'default_cli' && value)
                config.default_cli = value;
        ...
        if (currentSection === 'commander' && indent === 2) {
            ...
            if (config.commander) {
                if (key === 'identity')
                    config.commander.identity = value;
                if (key === 'brain')
                    config.commander.brain = value;
            }
            continue;
        }
        // agents セクション
        ...
        if (currentSection === 'agents' && currentKey && indent >= 4) {
            ...
            if (key === 'model_primary')
                agent.model_primary = value;
            if (key === 'model_backup')
                agent.model_backup = value;
            if (key === 'model_backups')
                agent.model_backups = value.split(',').map(s => s.trim());
            if (key === 'cli_backup')
                agent.cli_backup = value;
            if (key === 'cli_backup_model')
                agent.cli_backup_model = value;
            continue;
        }
        ...
        if (currentSection === 'definitions' && currentKey && indent >= 4) {
            ...
            if (k === 'command' && value)
                def.command = value;
            if (k === 'subcommand')
                def.subcommand = value || undefined;
            if (k === 'prompt_flag')
                def.prompt_flag = value;
            if (k === 'auto_approve')
                def.auto_approve = value;
            if (k === 'output_format')
                def.output_format = value;
            if (k === 'workspace_flag')
                def.workspace_flag = value;
            if (k === 'resume_flag')
                def.resume_flag = value;
        }
    }
    return config;
}
function getDefaultConfig() {
    return {
        ...
        agents: {
            ...
            developer: {
                cli: 'opencode',
                model_primary: 'github-copilot/gpt-5-mini',
                model_backups: ['github-copilot/claude-sonnet-4.6', 'github-copilot/gpt-5.4'],
                cli_backup: 'copilot',
                cli_backup_model: 'github-copilot/gpt-5-mini'
            },
 ...
 */
export function resolveCli(skill, config, override) {
 ...
 * CLI実行コマンドを構築する。
 */
export function buildCommand(definition, prompt, options = {}) {
    ...
    if (options.model) {
        ...
        if (cmd === 'copilot') {
            normalizedModel = normalizedModel.replace('github-copilot/', '').replace(/\s+/g, '-').toLowerCase();
        }
        else if (cmd === 'opencode') {
            normalizedModel = normalizedModel.replace(/\s+/g, '-').toLowerCase();
        }
        parts.push(`--model=${normalizedModel}`);
    }
    // ワークスペース
 ...
 */
export function buildCommandArgs(definition, prompt, options = {}) {
    const cmd = definition.command;
    const args = [];
    // サブコマンド（opencode run）
    if (definition.subcommand) {
    ...
    if (options.model) {
        let normalizedModel = options.model;
        if (cmd === 'copilot') {
            // Copilot CLI は ID のみを受け付けるため、サービスプレフィックスを除去し、スペースをハイフンに変換
            normalizedModel = normalizedModel.replace('github-copilot/', '').replace(/\s+/g, '-').toLowerCase();
        }
        else if (cmd === 'opencode') {
            // Opencode は provider/model 形式だが、スペースはハイフンにする必要がある
            normalizedModel = normalizedModel.replace(/\s+/g, '-').toLowerCase();
        }
 ...
 * 利用可能なCLIを検出する。
 */
export function detectAvailableClis(config) {
    const results = [];
...
agents:
  reviewer:
    cli: copilot
    model_primary: github-copilot/claude-opus-4.6
    model_backups: github-copilot/gpt-5.4, github-copilot/gpt-5 mini
    cli_backup: gemini
    cli_backup_model: gemini-3-flash-preview
`;
    const config2 = parseYaml(agentYaml);
    const agent = config2.agents?.['reviewer'];
    assert(!!(agent?.cli === 'copilot'), 'agent.cli = copilot');
    assert(!!(agent?.model_primary === 'github-copilot/claude-opus-4.6'), 'agent.model_primary');
    assert(!!Array.isArray(agent?.model_backups), 'agent.model_backups は配列');
    assert(!!agent?.model_backups?.includes('github-copilot/gpt-5.4'), 'agent.model_backups 含む');
    assert(!!(agent?.cli_backup === 'gemini'), 'agent.cli_backup = gemini');
    assert(!!(agent?.cli_backup_model === 'gemini-3-flash-preview'), 'agent.cli_backup_model');
    // Test 7: Model Normalization
    console.log('\n  [7/7] Model Normalization...');
    const cmd3 = buildCommand(config.cli_definitions['copilot'], 'test', { model: 'github-copilot/GPT-5 mini' });
    assert(cmd3.includes('--model=gpt-5-mini'), 'Copilot model normalization (strip prefix, space to hyphen)');
    const cmd4 = buildCommand(config.cli_definitions['opencode'], 'test', { model: 'github-copilot/GPT-5 mini' });
    assert(cmd4.includes('--model=github-copilot/gpt-5-mini'), 'Opencode model normalization (space to hyphen)');
    console.log(`\n═══════════════════════════════════════════`);
