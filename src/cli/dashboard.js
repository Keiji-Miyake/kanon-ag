...
class LogManager {
    ...
    // スキルごとの色割り当て
    skillColors = {};
    colorPalette = [ANSI.cyan, ANSI.magenta, ANSI.blue, ANSI.yellow, ANSI.green];
    nextColorIdx = 0;
...
export class DashboardInteraction {
    ...
    async sendIntervention(message) {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
 ...
 * ダッシュボード画面を文字列として生成
 */
export function renderDashboard(session, rows, logs, width = 70, height = 24) {
    const lines = [];
...
// ─── Data Loading ───────────────────────────────────────────
function loadSession(memoriesDir) {
...
function loadSkillRows(memoriesDir) {
    const rows = [];
...
function extractLastTurn(content) {
...
function loadCliMapping(memoriesDir) {
...
function loadMetrics(memoriesDir) {
...
// ─── Live Dashboard ─────────────────────────────────────────
async function startLiveDashboard(config) {
...
function selfTest() {
    ...
    const output = renderDashboard({ id: 'test-123', status: 'RUNNING', phase: 'Phase 1' }, [], logs, 70, 20);
