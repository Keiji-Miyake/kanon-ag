...
export interface ConfigAgent {
    ...
    model_backup?: string; // バックアップモデル
    model_backups?: string[]; // 複数のバックアップモデルのシークエンス
    cli_backup?: string;   // バックアップCLI
    cli_backup_model?: string; // バックアップCLI使用時のモデル
}
...
export interface CliConfig {
    ...
    agents: Record<string, ConfigAgent>; // 必須に変更
    skill_cli_mapping: Record<string, string>;
    agentModels?: Record<string, string>; // 追加: テスト互換用
...
export interface BuildCommandOptions {
    ...
    skipPrompt?: boolean;   // プロンプトを引数に含めない（stdinで渡す用）
}
...
export function mergeJsonConfig(base: CliConfig, json: Record<string, unknown>): CliConfig {
    ...
    if (json.agentModelMapping && typeof json.agentModelMapping === 'object') {
        const mapping = json.agentModelMapping as Record<string, string>;
    ...
    if (json.agents && typeof json.agents === 'object') {
        for (const [role, agentCfg] of Object.entries(json.agents as Record<string, unknown>)) {
            ...
            merged.agents[role] = {
                ...
                cli_backup_model: typeof cfg.cli_backup_model === 'string' ? cfg.cli_backup_model : ((current as any).cli_backup_model || (defaultConfig as any).cli_backup_model),
            };

            // 後方互換性のためマッピングも更新
            merged.skill_cli_mapping[role] = cli;
            if (!merged.agentModels) merged.agentModels = {};
...
function parseYaml(content: string): CliConfig {
    ...
    for (const rawLine of lines) {
        ...
        if (currentSection === 'agents' && currentKey && indent >= 4) {
            ...
            if (key === 'model_backups') agent.model_backups = value.split(',').map(s => s.trim());
            if (key === 'cli_backup') agent.cli_backup = value;
            if (key === 'cli_backup_model') agent.cli_backup_model = value;
            continue;
        }
