...
export function initSession(sessionId, workspace) {
    ...
    fs.writeFileSync(path.join(memoriesDir, 'metrics.json'), JSON.stringify(metrics, null, 2), 'utf-8');
    return session;
}
function saveSession(session, workspace) {
    const base = getBaseDir(workspace);
    const memoriesDir = path.join(base, MEMORIES_DIR);
...
export function updateSession(status, phase, workspace, command, targetTask, currentPassage, worktreePath) {
    const base = getBaseDir(workspace);
    ...
    if (currentPassage !== undefined)
        session.currentPassage = currentPassage;
...
export function readSession(workspace) {
    ...
    const session = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));
...
// ─── Task Board ─────────────────────────────────────────────
export function updateTaskBoard(tasks, workspace) {
    const base = getBaseDir(workspace);
    const memoriesDir = path.join(base, MEMORIES_DIR);
    ensureDir(memoriesDir);
    // JSON 保存
...
export function readTaskBoard(workspace) {
    const base = getBaseDir(workspace);
...
// ─── Progress ───────────────────────────────────────────────
export function appendProgress(skill, turn, content, workspace) {
    const base = getBaseDir(workspace);
...
export function readProgress(skill, workspace) {
    const base = getBaseDir(workspace);
...
// ─── Results ────────────────────────────────────────────────
export function writeResult(skill, data, workspace) {
    const base = getBaseDir(workspace);
...
export function readResult(skill, workspace) {
    const base = getBaseDir(workspace);
...
// ─── Metrics ────────────────────────────────────────────────
export function updateMetrics(skill, data, workspace) {
    const base = getBaseDir(workspace);
...
export function readMetrics(workspace) {
    const base = getBaseDir(workspace);
    const metricsPath = path.join(base, MEMORIES_DIR, 'metrics.json');
    if (!fs.existsSync(metricsPath)) {
        return null;
...
// ─── Cleanup ────────────────────────────────────────────────
export function cleanMemories(workspace) {
...
// ─── Markdown Formatters ────────────────────────────────────
function formatSessionMd(session) {
    const statusIcon = {
        initializing: '🔄',
        running: '▶️',
        completed: '✅',
...
function formatTaskBoardMd(tasks) {
    let md = `# Task Board\n\n`;
...
async function selfTest() {
    ...
    try {
        ...
        assert(fs.existsSync(path.join(testDir, MEMORIES_DIR, 'session.md')), 'session.md 存在');
        assert(fs.existsSync(path.join(testDir, MEMORIES_DIR, 'task-board.md')), 'task-board.md 存在');
        assert(fs.existsSync(path.join(testDir, MEMORIES_DIR, 'metrics.json')), 'metrics.json 存在');
        ...
        updateMetrics('conductor', { elapsedMs: 135000, turns: 5, retries: 0, status: 'success' }, testDir);
        ...
        assert(!fs.existsSync(path.join(testDir, MEMORIES_DIR)), '.memories/ 削除済み');
