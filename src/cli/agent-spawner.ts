...
export interface SpawnConfig {
    ...
    /** リトライ遅延（ms配列、非推奨: retryConfigを使用） */
        ...
        for (const task of tasks) {
            ...
            if (agent.status !== 'running') {
                if (agent.status === 'completed') {
                    ...
                    if (config.retryConfig && retries < config.retryConfig.maxRetries) {
                        const delay = config.retryConfig.initialDelayMs * Math.pow(config.retryConfig.backoffFactor, retries);
                        retryCountMap.set(task.skill, retries + 1);
                        agentRegistry.delete(registryKey(sessionId, task.skill));
                        await sleep(delay);
                    } else if (config.retryDelaysMs && retries < config.retryDelaysMs.length) {
