...
describe('FeedbackInjector', () => {
    ...
    it('source: "user" の Issue が含まれる場合、[USER INTERVENTION] が objective に付与される', () => {
        const issues: Issue[] = [
            { level: 'error', description: 'ユーザーの指示', source: 'user' },
        ];
        const result = injector.injectIssues(baseInstruction, issues);
        expect(result.objective).toBe('[USER INTERVENTION] オセロゲームを実装する');
        const lastTask = result.tasks[result.tasks.length - 1];
        expect(lastTask).toContain('CRITICAL');
        expect(lastTask).toContain('[USER DIRECT INSTRUCTION]');
    });
